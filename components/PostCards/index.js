...
export default function PostCards({ highlightedPosts, BASE_URL }) {
    ...
    const fetchRecentPosts = async () => {
        try {
            setLoading(true)
            const res = await fetch(`${BASE_URL}/api/posts?sort=createdAt%3Adesc&pagination[page]=${pageNumber}&pagination[pageSize]=${PAGE_ENTRIES}&populate=*`);
    ...
    const setFeaturedItem = (type, index) => {
        if(type === 'img') {
            const imgUrl = highlightedPosts[index]?.attributes?.thumbnail?.data?.attributes?.url;
            return imgUrl ? `${BASE_URL}${imgUrl}` : FallbackImage.src;
        }
        if(type === 'title') {
